...
def draw(top9,user,lst,network,arOrAl):
	fu = open('list1.txt', 'w',encoding='utf-8') 
	fu.write("Opening draw \n")
	fu.write(str(top9))
	file_names = []
	...
	for item in top9:
		if arOrAl == "al":
			try:
				album_image = item.get_cover_image(pylast.COVER_EXTRA_LARGE)
				if album_image == None:
		...
		if arOrAl == "al":
			file_names.append([file_name, item.artist.name, item.title])
		if arOrAl == "ar":
			file_names.append([file_name, item.name])
	...
	for i in range(0, 900, 300):
		for j in range(0, 900, 300):
			...
			if arOrAl == "al":
				length = "{} Min".format(str(int(lst[file_names[array_position][2]][0]/60)))
			if arOrAl == "ar":
				length = "{} Min".format(str(int(lst[file_names[array_position][1]]/60)))
